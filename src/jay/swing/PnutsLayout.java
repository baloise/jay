...
public class PnutsLayout implements LayoutManager2, Serializable {
	...
	private void bindContainer(Container target) {
		...
		for (int i = 0, gridX = 0, gridY = 0; i < compCount; i++) {
			...
			CellConstraints constraints = (CellConstraints) cellConstraints
					.get(comp);
		...
		for (int i = 0; i < compCount; i++) {
			...
			int padding[] = constraints.getPadding().clone();
			int spacing[] = constraints.getSpacing().clone();
			...
			// Store bottom and right spacing at all grid locations component
			// occupies.
			...
			if (colspan == 1) {
				...
				for (int ii = 0; ii < colspan; ii++) {
					if ((xExpand[gridX + ii] & 1) != 0) { // expand is set
						int expandedWidth = (compWidth - no_expand_widths)
								/ expand_count;
			...
			if (heights[gridY] < (compHeight + padding[0] + padding[2])
					/ rowspan) {
				heights[gridY] = (compHeight + padding[0] + padding[2])
						/ rowspan;
		...
		xExpandCount = 0;
		for (int element : xExpand) {
			if ((element & 1) != 0) {
				xExpandCount++;
			}
		}
		yExpandCount = 0;
		for (int element : yExpand) {
			if ((element & 1) != 0) {
				yExpandCount++;
			}
		}
	 ...
	 * in the target in order to satisfy the constraints of the PnutsLayout
	 * object.
	 * 
	 * @see Container
	 */
	public void layoutContainer(Container target) {
		...
		for (int i = 0; i < compCount; i++) {
			...
			if (fill[0] > 0) {
				compWidth = (int) ((columnLeft[gridX + colspan]
						- columnLeft[gridX] - (padding[3] + padding[1])) * (fill[0] / 100F));
			}
			if (fill[1] > 0) {
			...
			if ((align & LEFT) != 0) {
				...
				x = (columnLeft[gridX] + columnLeft[gridX + colspan]
						- compWidth + padding[3] - padding[1]) / 2;
			}
			...
			if ((align & TOP) != 0) {
				...
				y = (rowTop[gridY] + rowTop[gridY + rowspan] - compHeight
	...
	/**
	 * Returns the constraints used by the table and the default constraints
	 * used for all cells in the table. Modifying this object will modify the
	 * table's constraints.
	 */
	...
	public Rectangle getGridRectangle(Container parent, int index) {
		...
		CellConstraints constraints = (CellConstraints) cellConstraints
				.get(parent.getComponent(index));
		int colspan = constraints.getColspan();
		int rowspan = constraints.getRowspan();
		return new Rectangle(cellWidth, cellHeight, columnLeft[gridX + colspan]
				- cellWidth, rowTop[gridY + rowspan] - cellHeight);
	}
	...
	public void addLayoutComponent(Component comp, Object obj) {
		if (obj == null) {
			setConstraints(comp, Collections.EMPTY_MAP);
	 ...
	 * information it should be discarded. This happens automatically when cell
	 * or table constraints are modified.
	 */
		...
		while (constraints.hasMoreTokens()) {
			...
			if (constraintString.indexOf('=') == -1) {
				throw new LayoutException(
						"Invalid constraint name/value pair: " + token);
			}
	 ...
	 * @param obj
	 *            Must be a String.
	 * @return An Integer representing the passed in String, or null if the
	 *         String is null.
	...
	public abstract class Constraints {
		...
		/**
		 * Configures this contraint object with a Map. See PnutsLayout class
		 * comments for constraints usage.
		 */
		public void configure(Map map) {
			...
			if (map.get("align") == null) {
				...
				if ("fill".equals(valign)) {
					setFill(new Integer[] { getInteger(getFill()[0]),
							getInteger(100) });
					valign = null;
				}
				String halign = (String) map.get("halign");
				if ("fill".equals(halign)) {
					setFill(new Integer[] { getInteger(100),
							getInteger(getFill()[1]) });
					halign = null;
				}
		...
		public int[] getPadding() {
			...
			return new int[] { padding[0].intValue(), padding[1].intValue(),
					padding[2].intValue(), padding[3].intValue() };
		}
		...
		public int[] getSpacing() {
			...
			return new int[] { spacing[0].intValue(), spacing[1].intValue(),
					spacing[2].intValue(), spacing[3].intValue() };
		}
	...
	/**
	 * Stores constraints for the table and default constraints for the table
	 * cells. These defaults are used for all cells that don't specify a value.
	 * This means TableConstraints fields canont be set to null.
	 */
